...
(function(angular){
'use strict';
angular.module("components").factory("errorResolverInterceptor", ["$q", function($q) {
...
$templateCache.put('./side-nav.component.html','<ul id="slide-out" class="side-nav"><li><div class="userView"><div class="background cyan darken-2"><!-- <img src="assets/img/bg.jpg"> --></div><a href="{{$ctrl.user.link}}"><img class="circle" ng-src="{{$ctrl.user.profile_image}}"></a><a href="{{$ctrl.user.link}}"><span class="white-text name"><b>{{$ctrl.user.display_name}}</b></span></a> <a><span class="white-text name aqt-nav-site"><small>stack Overflow em Portugu\xEAs</small></span></a></div></li><li><a href="#/new"><i class="material-icons">mode_edit</i>{{\'LB_MENU_NEW_Q\' | translate}}</a></li><li><div class="divider"></div></li><li><a href="#/main"><i class="material-icons">view_list</i>{{\'LB_MENU_MY_Q\' | translate}}</a></li><li><div class="divider"></div></li><li><a href="#/login" ng-click="$ctrl.signout()"><i class="material-icons">power_settings_new</i>{{\'LB_MENU_OUT\' | translate}}</a></li></ul>');
...
$templateCache.put('./main.html','<side-nav></side-nav><center><h4>{{ \'LB_MAIN_TITLE\' | translate }}</h4></center><div ng-repeat="question in mainCtrl.questions"><div class="row"><div class="col s12 m6"><div class="card"><div class="card-content"><span class="card-title">{{question.title}}</span><p class="card-subtitle" ng-repeat="tag in question.tags"><a><span class="new badge" data-badge-caption="{{tag}}"></span></a></p></div><div align="center" class="card-action"><a href="{{question.link}}">{{ \'BT_MAIN_AUTH\' | translate }}</a></div></div></div></div></div><btn-fb url="\'#/new\'" icon="add"></btn-fb>');
